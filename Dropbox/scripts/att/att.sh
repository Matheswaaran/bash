...
for fl in `ls data/$yr-$sc-*|sort -V`;do ddat=$(cat $fl) && nbr=$(ls $fl|awk -F'[-|.]' '{print $3}') && for ni in {1..6};do dary[$ni]=-;done
...
if [[ $md == 1 ]] && [[ ${ip1:0:1} == "S" ]] && [[ ${ip1:1:1} == [1-9] ]] && [[ $(echo $ip1|awk '{print $2}'|wc -L) -le 2 ]];then tp=$ip1 && echo $tp|while read y1 y2;do for ni in `echo $y2|sed 's/,/\ /g'|sed 's/[A-Z]//g'`;do ip1="$ni $(echo $y1|sed 's/[A-Z07-9]//g')" && adapndcl;done;done && ip1=$tp && er=0 && jb=1;else :;fi
if [[ $md == 1 ]] && [[ ${ip1:0:1} == "S" ]] && [[ ${ip1:1:1} == "E" ]] && [[ ${ip1:2:1} == [1-9] ]] && [[ $(echo $ip1|awk '{print $2}'|wc -L) -le 2 ]];then tp=$ip1 && echo $tp|while read y1 y2;do for ni in `echo $y2|sed 's/,/\ /g'|sed 's/[A-Z]//g'`;do ip1="$ni $(echo $y1|sed 's/[A-Z07-9]//g')" && delapndcl;done;done && ip1=$tp && er=0 && jb=1;else :;fi
...
if [[ $ip1 != "EXIT" ]];then :;else break;fi
